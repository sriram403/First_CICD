...
class ModelFactory:
    ...
    def class_for_name(module_name,class_name):
        try:
            module = importlib.import_module(module_name)#importing the modules(sklearn.linear_regression)
            logging.info(f"Executing command: from {module} import {class_name}")
            class_ref = getattr(module,class_name)#getting the attribute(model_name) from sklearn.linear_regression
            return class_ref
        except Exception as e:
            raise HousingException(e,sys) from e
    ...
    def update_property_of_class(instance_ref,property_data):
        try:
            if not isinstance(property_data,dict):
                raise Exception("yoo the parameter doesn't have a dictionary format to it :(")
            print(property_data)
    ...
    def get_initialized_model_list(self)->InitializedModelDetail:
        try:
    ...
    def initiate_best_parameter_search_for_initialized_model(self,initialized_model,input_feature,output_feature):
        try:
    ...
    def get_best_model(self,x,y,base_accuracy=0.6)->BestModel:
        try:
            ...
            logging.info(f"Initialized_model: [{initialized_model_list}]")
